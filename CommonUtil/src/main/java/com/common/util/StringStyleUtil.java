...
public final class StringStyleUtil {
    ...
    public static SpannableStringBuilder makeStyledString(Context context, String content, int colorId, int textSize) {
        SpannableStringBuilder string = new SpannableStringBuilder(content);
        string.setSpan(new ForegroundColorSpan(colorId), 0, string.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        string.setSpan(new AbsoluteSizeSpan(textSize), 0, string.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return string;
    }
